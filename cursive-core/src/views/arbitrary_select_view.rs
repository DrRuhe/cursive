...
use crate::{
    ...
    view::{CannotFocus, IntoBoxedView, Position, View},
...
/// ```
pub struct ArbitrarySelectView<Data = String> {
    // The core of the view: we store a list of items
    // `Item` is more or less a `(View, Rc<T>)`.
    items: Vec<Item<Data>>,
    ...
    cached_heights: Vec<Vec2>,
}

impl<Data: 'static> Default for ArbitrarySelectView<Data> {
    fn default() -> Self {
        Self::new()
    }
}

impl<Data: 'static> ArbitrarySelectView<Data> {
    ...
    pub fn selection(&self) -> Option<Rc<Data>> {
        let focus = self.focus();
        if self.len() <= focus {
            None
        } else {
            Some(Rc::clone(&self.items[focus].data))
        }
    }
    ...
    /// ```
    pub fn add_item<V: IntoBoxedView + 'static>(
        &mut self,
        view: V,
        data: Data,
    ) {
        self.items.push(Item::new(view, data));
    }
    ...
    /// ```
    pub fn get_item(&self, i: usize) -> Option<(&Box<dyn View>, &Data)> {
        self.iter().nth(i)
    }
    ...
    pub fn get_item_mut(
        &mut self,
        i: usize,
    ) -> Option<(&mut Box<dyn View>, &mut Data)> {
        if i >= self.items.len() {
            None
        } else {
            let item = &mut self.items[i];
            if let Some(t) = Rc::get_mut(&mut item.data) {
                let label = &mut item.view;
                Some((label, t))
            } else {
                None
            }
    ...
    pub fn iter_mut(
        &mut self,
    ) -> impl Iterator<Item = (&mut Box<dyn View>, &mut Data)>
    where
        Data: Clone,
    {
        self.items
            .iter_mut()
            .map(|item| (&mut item.view, Rc::make_mut(&mut item.data)))
    }
    ...
    pub fn try_iter_mut(
        &mut self,
    ) -> impl Iterator<Item = (&mut Box<dyn View>, Option<&mut Data>)> {
        self.items
            .iter_mut()
            .map(|item| (&mut item.view, Rc::get_mut(&mut item.data)))
    }

    /// Iterate on the items in this view.
    ///
    /// Returns an iterator with each item and their labels.
    pub fn iter(&self) -> impl Iterator<Item = (&Box<dyn View>, &Data)> {
        self.items.iter().map(|item| (&item.view, &*item.data))
    }
    ...
    /// the right.
    pub fn insert_item<V>(&mut self, index: usize, label: V, value: Data)
    where
        V: IntoBoxedView + 'static,
    {
        self.items.insert(index, Item::new(label, value));
    }
    ...
    /// ```
    #[must_use]
    pub fn item<V: IntoBoxedView + 'static>(
        self,
        label: V,
        value: Data,
    ) -> Self {
    ...
    pub fn add_all<V, I>(&mut self, iter: I)
    where
        V: IntoBoxedView + 'static,
        I: IntoIterator<Item = (V, Data)>,
    {
        for (s, t) in iter {
            self.add_item(s, t);
        }
    ...
    /// ```
    #[must_use]
    pub fn with_all<V, I>(self, iter: I) -> Self
    where
        V: IntoBoxedView + 'static,
        I: IntoIterator<Item = (V, Data)>,
    {
    ...
    {
        self.items.sort_by(|a, b| compare(&a.data, &b.data));
    }
    ...
    where
        ...
        K: Ord,
    {
        self.items.sort_by_key(|item| key_of(&item.data));
    }
            ...
            Event::Mouse {
                ...
                if let position =
                    position.checked_sub(offset).map(|position| {
                        ...
                        //let total_sizes = self.cached_heights.iter().sum().x;
                    })
                {
                }
                ...
                if position
                    .checked_sub(offset)
                    .map(|position| {
                        //TODO subtract all the cached sizes here.
                        todo!();
                ...
                //TODO get focus depending on heights
...
//TODO reenable
impl<Data> ArbitrarySelectView<Data>
where
    Data: IntoBoxedView + 'static + Clone,
{
    /// Convenient method to use the label as value.
    pub fn add_viewable_data(&mut self, label: Data) {
    ...
    /// Chainable Variant
    #[must_use]
    pub fn with_viewable_data(self, label: Data) -> Self {
        self.with(|s| s.add_viewable_data(label))
    }
    ...
    /// ```
    pub fn add_all_viewable_data<I>(&mut self, iter: I)
    where
        I: IntoIterator<Item = Data>,
    {
        for s in iter {
            self.add_viewable_data(s);
        }
    }
    ...
    /// ```
    #[must_use]
    pub fn with_all_viewable_data<I>(self, iter: I) -> Self
    where
        I: IntoIterator<Item = Data>,
    {
        self.with(|s| s.add_all_viewable_data(iter))
    }
}
    ...
    Data: Ord,
{
    ...
    pub fn sort(&mut self) {
        self.items.sort_by(|a, b| a.data.cmp(&b.data));
    }
}

impl<Data: 'static> View for ArbitrarySelectView<Data> {
    ...
    fn required_size(&mut self, req: Vec2) -> Vec2 {
        return direction::Orientation::Vertical.stack(
            self.items.iter_mut().map(|item| {
                let view = item.view.as_mut();
    ...
    fn layout(&mut self, size: Vec2) {
        //TODO calculate the sizes of the children so its available during the draw method.
        todo!();
        //self.last_size = size;
    }
...
impl<Data> Item<Data> {
    fn new<V: IntoBoxedView + 'static>(view: V, data: Data) -> Self {
...
#[cfg(test)]
mod tests {
    use crate::views::{TextArea, TextView};
    ...
    fn select_view_sorting_with_comparator() {
        ...
        let mut view = ArbitrarySelectView::new();
        view.add_item(TextView::new("Y"), 2);
        view.add_item(TextView::new("Z"), 1);
        view.add_item(TextView::new("X"), 3);
    ...
    fn select_view_sorting_by_key() {
        ...
        let mut view = ArbitrarySelectView::new();
        view.add_item(TextView::new("Y"), MyStruct { key: 2 });
        view.add_item(TextView::new("Z"), MyStruct { key: 1 });
        view.add_item(TextView::new("X"), MyStruct { key: 3 });
